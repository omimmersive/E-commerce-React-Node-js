...
const Test = () => {
  ...
  const { data, loading, error } = useAsync(TestServices.getAllTest);
  const { data: getAllTests } = useAsync(
    TestServices.getAllTests
  );
  ...
  return (
    <>
      ...
      <MainDrawer>
        <TestDrawer id={serviceId} data={data} lang={lang} />
      </MainDrawer>
      ...
      {loading ? (
        ...
        <NotFound title="Sorry, There are no test right now." />
      )}
    </>
  );
};
